...
y2 = interp1(1:N,y1./A,m,'spline');
[Fy2,~,~,~,~,~,omega2] = sstn_test_modL(y2,gamma,sigma,ft,bt);
%---------------------------------------

%-----------
c = exridge(Fy2,0,0,10);
frec = zeros(size(x));
for i = 1:N
    frec(i) = omega2(c(i),i); 
    A(i) = abs(Fy2(c(i),i)); 
...
y4 = interp1(1:N,y3./A,m,'spline');
[Fy4,~,~,~,~,~,omega2] = sstn_test_modL(y4,gamma,sigma,ft,bt);
%---------------------------------------
%-----------------
c = exridge(Fy4,0,0,10);
frec = zeros(size(x));
for i = 1:N
    frec(i) = omega2(c(i),i); 
    A(i) = abs(Fy4(c(i),i)); 
...
subplot(10,15,[6:15 21:30]); plot(t,x,'k'); xlim([50 100]); title('zoomed in original ABP signal'); xlabel('time [s]')
...
title('after 3 demodulations w/ 1st harm.');% text(1,-380,'ECG signal')
...
title('after 5 demodulations w/ 1st harm.');% text(1,-380,'ECG signal')
...
title('warped ABP signal')
xlim([0 40])
ylim([-1.2 2.5])
xlabel('warped time')
for i = 1:4
    plot([8*(i-1)+1 8*(i-1)+2],[1.8 1.8],colores(i));
    text(8*(i-1)+2.3,1.8,['waveform no. ' num2str(i)]);
end;
