aux = importdata('8-i_n.wav');
fs = aux.fs;
aux = aux.data;
aux = resample(aux,1,10); 
x = aux;

aux = importdata('8-u_l.wav');
aux = aux.data;
aux = resample(aux,1,10); 
x = [x;aux];

aux = importdata('8-i_lhl.wav');
aux = aux.data;
aux = resample(aux,1,10);
x = [x;aux];
...
f_m = 1/(aux(1)*N*redun):1/(aux(1)*N*redun):fmax/aux(1);
figure;
subplot(10,15,[1:5]); plot(t,x,'k'); set(gca,'xticklabel',[]); xlim([t(1) t(end)]); title('original signal')%text(2.3,0.75,'/a/')
...
set(gca,'yticklabel',[])
title('Clustering')
subplot(10,15,[87:90 ]); plot(0:2*pi/T:2*pi-2*pi/T,WSF(1,:),'k')
title('waveform no. 1')
%text(0.5,1.2,'waveform no. 1')
set(gca,'yticklabel',[])
xlabel('phase')
subplot(10,15,[102:105]); plot(0:2*pi/T:2*pi-2*pi/T,WSF(2,:),'k')
title('waveform no. 2')
%text(0.5,1.2,'waveform no. 2')
set(gca,'yticklabel',[])
xlabel('phase')
subplot(10,15,[117:120]); plot(0:2*pi/T:2*pi-2*pi/T,WSF(3,:),'k')
title('waveform no. 2')
%text(0.5,1.2,'waveform no. 2')
set(gca,'yticklabel',[])
xlabel('phase')
...
subplot(10,15,[6:15]); plot(t,x,'k'); xlim([0.4 0.6]);% ylim([-1 1])
title('zoomed in original signal')
xlabel('time [s]')

subplot(10,15,[21:30]); plot(t,x,'k'); xlim([1.4 1.6]);% ylim([-1 1])
title('zoomed in original signal')
xlabel('time [s]')

subplot(10,15,[36:45]); plot(t,x,'k'); xlim([2.65 2.85]);% ylim([-1 1])
title('zoomed in original signal')
xlabel('time [s]')

subplot(10,15,[51:60]); plot(t,x,'k'); xlim([3.4 3.6]);% ylim([-1 1])
title('zoomed in original signal')
xlabel('time [s]')
